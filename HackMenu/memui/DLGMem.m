...
- (void)nearMemSearch:(const char*)value type:(int)type range:(int)range{
    ...
    int count = 0;
    g_chain = near_mem_search_func(v,size,type,g_chain,&count,range);
    [self.memView setChainCount:count];
    [self.memView setChain:g_chain];
}
...
- (void)DLGMemUINearMemSearch:(NSString *)value type:(DLGMemValueType)type range:(int)range{
    const char *v = [value UTF8String];
    int t = [self memTypeFromDLGMemValueType:type];
    [self nearMemSearch:v type:t range:range];
}
