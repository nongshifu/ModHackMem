...
- (void)onModButtonTapped:(id)sender {
    if (self.modifying) {
        ...
        self.value = text;
        if ([self.delegate respondsToSelector:@selector(DLGMemUIViewCellModify:value:type:)]) {
            [self.delegate DLGMemUIViewCellModify:self.address value:self.value type:self.Type];
        }
    }
